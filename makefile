...
VERSION   ?= $(call get_gh_version,kanboard/kanboard)
BSVERSION ?= $(call get_gh_version,kanboard/plugin-beanstalk,BSVERSION)
...
TESTCMD   := \
	...
	php --version; \
	php-fpm -v; \
	echo "Kanboard version: $$(cat /opt/kanboard/version)"; \
	beanstalkd -v; \
	echo "Plugin-beanstalk version: $$(cat /opt/kanboard/plugin_beanstalk_version)"; \
	#
# -- }}}
...
BUILDFLAGS ?= \
	...
	--build-arg BSVERSION=$(if $(BSVERSION),$(BSVERSION),$(error BSVERSION is not defined)) \
...
# runtime flags
MOUNTFLAGS := \
	# -v $(CURDIR)/data/data:/config/www/kanboard/data \
	# -v $(CURDIR)/data/plugins:/config/www/kanboard/plugins \
	# -v $(CURDIR)/data/config.php:/config/www/kanboard/config.php \
...
get_config: ## get default config from github
	curl \
		-o $(CURDIR)/root/defaults/config.php \
		-SL https://raw.githubusercontent.com/kanboard/kanboard/refs/heads/main/config.default.php
